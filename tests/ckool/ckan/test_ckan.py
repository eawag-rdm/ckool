...
@pytest.mark.impure
def test_get_package(ckan_instance, ckan_entities, ckan_setup_data, valid_outputs):
    data = ckan_instance.get_package(ckan_entities["test_package"])
...
@pytest.mark.impure
def test_get_package_metadata_filtered(ckan_instance, ckan_entities, ckan_setup_data):
...
def test_reorder_package_resources(
    ...
    resource_names_initial = [
        r["name"]
        for r in ckan_instance.get_package(ckan_entities["test_package"], ["resources"])[
            "resources"
        ]
    ]
    ckan_instance.reorder_package_resources(ckan_entities["test_package"])
...
@pytest.mark.impure
def test_resource_patch(ckan_instance, ckan_entities, ckan_setup_data):
    new_name = "new_name"
    resource_id = ckan_instance.get_package(ckan_entities["test_package"])["resources"][
        0
    ]["id"]
    ckan_instance.patch_resource_metadata(
        resource_id=resource_id, resource_data_to_update={"name": new_name}
    )
    name_in_ckan = ckan_instance.get_package(ckan_entities["test_package"])["resources"][
        0
    ]["name"]
...
@pytest.mark.impure
def test_update_package_metadata(ckan_instance, ckan_entities, ckan_setup_data):
    data = ckan_instance.get_package(ckan_entities["test_package"])
    ...
    changed_data = ckan_instance.get_package(ckan_entities["test_package"])
...
@pytest.mark.impure
def test_patch_package_metadata(ckan_instance, ckan_entities, ckan_setup_data):
    data = ckan_instance.get_package(ckan_entities["test_package"])
    original_message = data["notes"]
    new_message = "this field was changed"
...
@pytest.mark.impure
def test_resource_create_link(ckan_instance, ckan_entities, ckan_setup_data):
...
def test_resource_create_file_minimal(
    ckan_instance, ckan_entities, ckan_setup_data, small_file
):
    ckan_instance.create_resource_of_type_file(
        **{
            "file": small_file,
...
def test_download_package_with_resources_sequential(
    tmp_path, ckan_instance, ckan_entities, ckan_setup_data, add_file_resources
):
    add_file_resources(
        [
            100 * 1024**2,
...
@pytest.mark.impure
def test_filter_resource(tmp_path, ckan_instance, ckan_entities, ckan_setup_data):
    for i in range(3):
...
@pytest.mark.impure
def test_organization_existence(ckan_instance, ckan_entities, ckan_setup_data):
    ckan_instance.get_organization(ckan_entities["test_organization"])
    with pytest.raises(ckanapi.errors.NotFound):
        ckan_instance.get_organization("does-not-exist")


@pytest.mark.impure
def test_get_group(ckan_instance, ckan_entities, ckan_setup_data):
    assert len(ckan_instance.get_project(ckan_entities["test_project"])["packages"]) == 0
    with pytest.raises(ckanapi.errors.NotFound):
        ckan_instance.get_project("absas")


@pytest.mark.impure
def test_add_package_to_project(ckan_instance, ckan_entities, ckan_setup_data):
    ckan_instance.add_package_to_project(
        ckan_entities["test_package"], ckan_entities["test_project"]
    )
    assert len(ckan_instance.get_project(ckan_entities["test_project"])["packages"]) == 1
    assert len(ckan_instance.get_package(ckan_entities["test_package"])["groups"]) == 1


@pytest.mark.impure
def test_get_user(ckan_instance, ckan_setup_data):
    assert ckan_instance.get_user("ckan_admin")


@pytest.mark.impure
def test_create_package_with_additional_field(
    ckan_instance, ckan_setup_data
):
...
@pytest.mark.impure
def test_package_delete_purge(ckan_instance, ckan_entities, ckan_setup_data):
    pkg = deepcopy(package_data)

    ckan_instance.delete_package(ckan_entities["test_package"])
    assert ckan_instance.get_package(ckan_entities["test_package"])["state"] == "deleted"

    ckan_instance.create_package(**pkg)
    ckan_instance.delete_package(ckan_entities["test_package"])
    ckan_instance.purge_package(ckan_entities["test_package"])
    with pytest.raises(ckanapi.errors.NotFound):
        ckan_instance.get_package(ckan_entities["test_package"])


@pytest.mark.impure
def test_package_delete_delete_purge(ckan_instance, ckan_entities, ckan_setup_data):
    ckan_instance.delete_package(ckan_entities["test_package"])
    ckan_instance.delete_package(ckan_entities["test_package"])
    assert ckan_instance.get_package(ckan_entities["test_package"])["state"] == "deleted"
    ckan_instance.purge_package(ckan_entities["test_package"])
